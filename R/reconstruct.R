...
#'   (continuous). This produces long-range forecasts and is always returned as
#'   `X_pred_arr`.
#'
...
#'   operator returned by `method = "continuous"` in
#'   [run_dmd()].  The continuous-time propagation uses the matrix
#'   exponential. Requires the \pkg{expm} package.
...
reconstruct_dmd <- function(
  ...
  if (is.null(n_steps)) {
    n_steps <- if (!is.null(X_obs)) {
      ...
      stop("reconstruct_dmd: n_steps must be specified when X_obs is not provided.")
    }
  }
  ...
  if (return_single_step && !is.null(X_obs)) {
    ...
    for (gi in seq_len(G)) {
      ...
      for (t in seq_len(N_obs - 1L) + 1) {
        if (is_continuous) {
          cs      <- .ct_step(A_g, NULL, dt_vec_1step[t])
